...
def forward_tran(x_img=None,label_index=None, number_of_classes=5,layer_name='resnet_v2_152/block4',Training=True):
        ...
        with sess: #开始一个会话
            ...
            #for step in range(1):    
                ...
                if ((step % 1200 ==0) and step>0) or (step +1) == MAX_STEP:
                    checkpoint_path = os.path.join(logs_train_dir,'model_Resnet152.ckpt')
...
def run_eval(val_loop=2,number_of_classes = 5,batch_size=40,sigma=0.6,checkpoint_dir=None,dataset_dir=None):
    ...
    with tf.Graph().as_default() as g:
        ...
        _,_,_,logits_adver,_ = forward_tran_advers(x_img=mask_conv_imgs,number_of_classes=number_of_classes,Training=False)
